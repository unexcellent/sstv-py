...
def test_to_mp3_file_round_trip(tmp_path: Path, image):
...
def test_to_mp3_file_matches_to_mp3(tmp_path: Path, image):
    path = tmp_path / "transmission.mp3"
...
def test_to_mp3_file_unwritable_path_raises(tmp_path: Path, image):
    with pytest.raises(OSError):
...
def test_to_mp3_file_unsupported_sample_rate_raises(tmp_path: Path, image):
    with pytest.raises(ValueError, match="sample_rate"):
        sstv.encode_to_mp3_file(
